...
function  [X, mean, variance, rho, var_noise, final_error] = AMP_ABS_GEN(Y, G, kappa, opt)
    ...
    while (t <= t_max)
        ...
        if (t>1)
            for i=1:M
                if (Y(i)<kappa)                   
                    Gmu(i)=(Y(i)-W(i))/(1e-10+V(i));
                else
                    Gmu(i)=(Y(i).*tanh(Y(i).*W(i)./(1e-10+V(i)))-W(i))./(1e-10+V(i));
                end
            end
        else
            Gmu=zeros(M,1);
        ...
        for i=1:M
                if (Y(i)<kappa)
                    this_Gmu(i)=(Y(i)-W(i))/(1e-10+V(i));
                   % dw_Gmu(i)=-1./(1e-6+V(i));
                   % dw_Gmu(i)=this_Gmu(i)^2;
                else
                    this_Gmu(i)=(Y(i).*tanh(Y(i).*W(i)./(1e-10+V(i)))-W(i))./(1e-10+V(i));;
                   % dw_Gmu(i)=this_Gmu(i)^2;
                   % dw_Gmu(i)=-(((Y(i)./(1e-6+V(i))).^2).*(1-(tanh(Y(i).*W(i)./(1e-6+V(i)))).^2)-1./(1e-6+V(i)));
                end
        end
        dw_Gmu=mean2(    this_Gmu.^2)*ones(M,1);
